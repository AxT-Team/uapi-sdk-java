...
public class Example {
  public static void main(String[] args) {
    ...
    String authorization = "Bearer sk-xxx"; // String | Bearer类型的API密钥认证头。例如：`Bearer sk-xxx`
    try {
      GetStatusRatelimit200Response result = apiInstance.getStatusRatelimit(authorization);
      System.out.println(result);
    } catch (ApiException e) {
      System.err.println("Exception when calling StatusApi#getStatusRatelimit");
...
public class Example {
  public static void main(String[] args) {
    ...
    StatusApi apiInstance = new StatusApi(defaultClient);
    String path = "/api/v1/image/motou"; // String | （可选）如果你想查询某个特定的端点，请提供它的路径，例如 '/api/v1/image/motou'。如果留空，则返回所有端点的统计列表。
    try {
      GetStatusUsage200Response result = apiInstance.getStatusUsage(path);
      System.out.println(result);
    } catch (ApiException e) {
      System.err.println("Exception when calling StatusApi#getStatusUsage");
      System.err.println("Status code: " + e.getCode());
      System.err.println("Reason: " + e.getResponseBody());
      System.err.println("Response headers: " + e.getResponseHeaders());
      e.printStackTrace();
