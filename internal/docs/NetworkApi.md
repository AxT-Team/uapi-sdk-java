...
public class Example {
  public static void main(String[] args) {
    ...
    String source = "commercial"; // String | 查询结果类型。不传时返回标准结果；如果设置为 `commercial`，将返回更完整的地理位置信息，但响应时间可能会稍长。
    try {
...
| **source** | **String**| 查询结果类型。不传时返回标准结果；如果设置为 &#x60;commercial&#x60;，将返回更完整的地理位置信息，但响应时间可能会稍长。 | [optional] [enum: commercial] |
...
public class Example {
  public static void main(String[] args) {
    ...
    String source = "commercial"; // String | 查询结果类型。不传时返回标准结果；如果设置为 `commercial`，将返回更完整的地理位置信息，但响应时间可能会稍长。
    try {
      GetNetworkMyip200Response result = apiInstance.getNetworkMyip(source);
      System.out.println(result);
    } catch (ApiException e) {
