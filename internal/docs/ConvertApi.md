...
import org.openapitools.client.api.ConvertApi;
...
public class Example {
  public static void main(String[] args) {
    ...
    String time = "1698380645"; // String | 一个智能时间参数，可传入Unix时间戳（10位或13位）或标准日期字符串（如 '2023-10-27 10:30:00'），系统将自动识别并转换。
    try {
      GetConvertUnixtime200Response result = apiInstance.getConvertUnixtime(time);
      System.out.println(result);
    } catch (ApiException e) {
      System.err.println("Exception when calling ConvertApi#getConvertUnixtime");
...
public class Example {
  public static void main(String[] args) {
    ...
    ConvertApi apiInstance = new ConvertApi(defaultClient);
    PostConvertJsonRequest postConvertJsonRequest = new PostConvertJsonRequest(); // PostConvertJsonRequest | 这是一个JSON对象，里面必须包含一个名为 `content` 的字段。这个字段的值，就是你希望格式化的、原始的JSON字符串。
    try {
      PostConvertJson200Response result = apiInstance.postConvertJson(postConvertJsonRequest);
      System.out.println(result);
    } catch (ApiException e) {
      System.err.println("Exception when calling ConvertApi#postConvertJson");
      System.err.println("Status code: " + e.getCode());
      System.err.println("Reason: " + e.getResponseBody());
      System.err.println("Response headers: " + e.getResponseHeaders());
      e.printStackTrace();
