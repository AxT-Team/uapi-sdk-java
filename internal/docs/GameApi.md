...
public class Example {
  public static void main(String[] args) {
    ...
    try {
      GetGameEpicFree200Response result = apiInstance.getGameEpicFree();
      System.out.println(result);
    } catch (ApiException e) {
      System.err.println("Exception when calling GameApi#getGameEpicFree");
...
public class Example {
  public static void main(String[] args) {
    ...
    GameApi apiInstance = new GameApi(defaultClient);
    String name = "ExamplePlayer"; // String | 玩家的 Minecraft 用户名。使用此参数查询时，会返回所有匹配用户的列表（包括当前用户名或曾用名匹配的玩家）。
    String uuid = "ee9b4ed1-aac1-491e-b761-1471be374b80"; // String | 玩家的 Minecraft UUID，支持带连字符或不带连字符格式。
    try {
      GetGameMinecraftHistoryid200Response result = apiInstance.getGameMinecraftHistoryid(name, uuid);
      System.out.println(result);
    } catch (ApiException e) {
      System.err.println("Exception when calling GameApi#getGameMinecraftHistoryid");
      System.err.println("Status code: " + e.getCode());
      System.err.println("Reason: " + e.getResponseBody());
      System.err.println("Response headers: " + e.getResponseHeaders());
      e.printStackTrace();
...
public class Example {
  public static void main(String[] args) {
    ...
    GameApi apiInstance = new GameApi(defaultClient);
    String server = "hypixel.net"; // String | Minecraft服务器的地址，可以是域名（如 `hypixel.net`）或 `IP:端口` 的形式（如 `mc.example.com:25565`）。如果省略端口，将默认使用 `25565`。
    try {
      GetGameMinecraftServerstatus200Response result = apiInstance.getGameMinecraftServerstatus(server);
      System.out.println(result);
    } catch (ApiException e) {
      System.err.println("Exception when calling GameApi#getGameMinecraftServerstatus");
      System.err.println("Status code: " + e.getCode());
      System.err.println("Reason: " + e.getResponseBody());
      System.err.println("Response headers: " + e.getResponseHeaders());
      e.printStackTrace();
...
public class Example {
  public static void main(String[] args) {
    ...
    GameApi apiInstance = new GameApi(defaultClient);
    String username = "Notch"; // String | 玩家的 Minecraft 游戏内名称（正版ID）。
    try {
      GetGameMinecraftUserinfo200Response result = apiInstance.getGameMinecraftUserinfo(username);
      System.out.println(result);
    } catch (ApiException e) {
      System.err.println("Exception when calling GameApi#getGameMinecraftUserinfo");
      System.err.println("Status code: " + e.getCode());
      System.err.println("Reason: " + e.getResponseBody());
      System.err.println("Response headers: " + e.getResponseHeaders());
      e.printStackTrace();
...
public class Example {
  public static void main(String[] args) {
    ...
    GameApi apiInstance = new GameApi(defaultClient);
    String steamid = "76561197960287930"; // String | 用户的 Steam 标识。可以是以下任意一种格式： - 纯数字的 **SteamID64** - 用户的 **自定义 URL 名称** (Vanity URL) - 完整的 **个人资料链接** (包含 SteamID64 或自定义名称) - 好友代码 (如 `22202`)
    String id = "gabelogannewell"; // String | 用户的 Steam 自定义URL名称（Vanity URL）。例如个人资料链接中 `/id/` 后面的部分。
    String id3 = "STEAM_0:0:22202"; // String | 用户的 Steam ID3 格式标识符。传统的 Steam ID 格式，形如 STEAM_X:Y:Z。
    String key = "key_example"; // String | 你的 Steam Web API Key。这是一个可选参数，如果提供，它将覆盖我们在后端配置的全局Key。这为你提供了更大的灵活性，但请务必注意Key的保密，不要在前端暴露。
    try {
      GetGameSteamSummary200Response result = apiInstance.getGameSteamSummary(steamid, id, id3, key);
      System.out.println(result);
    } catch (ApiException e) {
      System.err.println("Exception when calling GameApi#getGameSteamSummary");
      System.err.println("Status code: " + e.getCode());
      System.err.println("Reason: " + e.getResponseBody());
      System.err.println("Response headers: " + e.getResponseHeaders());
      e.printStackTrace();
